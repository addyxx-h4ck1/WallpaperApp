const HDFunc = () => {
    fetch('database/hd-collection.json')
    .then(res => {
      return res.json();
    })
    .then(res => {
    ...
    let a = res.hdCollection;

    shuffleArray(a);
      /////////////////append data//////////////////////////
  
      const parent = document.getElementById('hdblock')
  ...
  //////////////load more////////////
    let loadMore = document.querySelector('.load-more-hd');
    let currentItem = 8;
    const b = document.querySelectorAll('#hdblock img')
  
    loadMore.onclick = () =>{
      let boxes = [...document.querySelectorAll('#hdblock img')];
